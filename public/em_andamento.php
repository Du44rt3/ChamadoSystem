...
<head>
    <meta charset="UTF-8">
    <meta name="                            <input type="text" name="pesquisa" class="form-control border-start-0" 
                                   placeholder="Pesquisar por código, colaborador, email, setor ou descrição..." 
                                   value="<?php echo isset($_GET['pesquisa']) ? htmlspecialchars($_GET['pesquisa']) : ''; ?>";wport" content="width=device-width, initial-scale=1.0">
    <title>Chamados Em Andamento - ELUS Facilities</title>
...
<body>
    ...
    <div class="container-fluid mt-4">
        ...
        <div class="search-bar">
            <div class="row">
                <div class="col-md-10">
                    <form method="GET" action="em_andamento.php" class="d-flex">
                        <div class="input-group">
                            ...
                            <?php if(isset($_GET['pesquisa']) && !empty($_GET['pesquisa'])): ?>
                                <a href="em_andamento.php" class="btn btn-secondary">
        ...
        if(isset($_GET['pesquisa']) && !empty($_GET['pesquisa'])){
            $stmt = $chamado->searchByStatus($_GET['pesquisa'], 'em_andamento');
            ...
            $stmt = $chamado->readByStatus('em_andamento');
        }
        ...
        if($num > 0){
            ...
            echo "<h5 class='mb-0'><i class='fas fa-clock me-2 text-warning'></i>Chamados Em Andamento <span class='badge bg-warning text-dark ms-2'>{$num}</span></h5>";
            ...
            foreach($rows as $row){
                ...
                echo "</a>";
                echo "</div>";
                echo "<span class='status-badge status-em_andamento'><i class='fas fa-cog fa-spin me-1'></i>Em Andamento</span>";
                echo "</div>";
                ...
                echo "<small>{$data_formatada} às {$hora_formatada}</small>";
                echo "</div>";
                echo "</div>";
                ...
                echo "<p class='card-text text-muted'>" . htmlspecialchars(substr($descricao_problema, 0, 120)) . (strlen($descricao_problema) > 120 ? '...' : '') . "</p>";
                echo "</div>";
                ...
                // SLA com possibilidade de edição
                echo "<div class='mb-3'>";
                echo "<div class='sla-info'>";
                echo "<div class='d-flex align-items-center mb-2'>";
                echo "<i class='fas fa-clock text-muted me-2'></i>";
                echo "<strong>SLA:</strong>";
                echo "<span class='ms-2 sla-prazo {$status_sla}' " . ($status_sla == 'critico' ? 'style="animation: blink 1s infinite;"' : '') . ">";
                echo $sla_real;
                echo "</span>";
                echo "</div>";
                if($data_limite_sla) {
                    ...
                    echo "<small class='text-muted'>Prazo: " . date('d/m/Y H:i', strtotime($data_limite_sla)) . "</small>";
                    echo "</div>";
                }
                echo "</div>";
                echo "</div>";
                ...
                echo "<i class='fas fa-tasks text-warning me-2'></i>";
                echo "<small class='text-warning fw-bold'>SENDO PROCESSADO</small>";
                echo "</div>";
                echo "</div>";
            ...
            echo "<table class='table table-hover mb-0' style='min-width: 1400px;'>";
            echo "<thead>";
            echo "<tr>";
            ...
            foreach($rows as $row){
                ...
                // Truncar descrição de forma inteligente
                ...
                echo "<td class='text-center'>";
                echo "<span class='sla-prazo {$status_sla}' " . ($status_sla == 'critico' ? 'style="animation: blink 1s infinite;"' : '') . ">";
                echo $sla_real;
                echo "</span>";
                if($data_limite_sla) {
                ...
                echo "</td>";
                echo "<td class='text-center'><span class='status-badge status-em_andamento'><i class='fas fa-cog fa-spin me-1'></i>Em Andamento</span></td>";
                echo "<td class='text-center'>";
                echo "<div class='btn-group btn-group-sm' role='group'>";
            ...
            echo "<div class='d-flex justify-content-center align-items-center mt-4 p-3 bg-warning bg-opacity-10 border border-warning rounded'>";
            echo "<small class='text-warning'><i class='fas fa-spinner me-2'></i>Total de <strong>{$num}</strong> chamado(s) em atendimento</small>";
            echo "</div>";
            ...
            if(isset($_GET['pesquisa']) && !empty($_GET['pesquisa'])){
                ...
                echo "<div>";
                echo "<strong>Nenhum chamado em andamento encontrado</strong><br>";
                echo "<small>Tente ajustar os termos da pesquisa ou <a href='em_andamento.php' class='alert-link'>ver todos os chamados em andamento</a></small>";
                echo "</div>";
                echo "</div>";
            } else {
                echo "<div class='alert alert-info d-flex align-items-center'>";
                ...
                echo "<div>";
                echo "<strong>Nenhum chamado em andamento</strong><br>";
                echo "<small>Todos os chamados estão aguardando início ou foram finalizados. <a href='add.php' class='alert-link'>Criar novo chamado</a></small>";
                echo "</div>";
                echo "</div>";
            }
        }
